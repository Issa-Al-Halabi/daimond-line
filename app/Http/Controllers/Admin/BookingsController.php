...
class BookingsController extends Controller
{
  ...
  // assign outcity trip for to driver 
    ...
    $query->where('user_type','driver')
        ...
        return view("bookings.view_event", compact('drivers','booking'));
    }
    ...
    public function index()
    {
        ...
        $booking = DB::table('bookings')->select('bookings.*', 'users.first_name', 'users.last_name','subcategories.title' ,'vehicles.car_model', 'vehicle_types.vehicletype')
    ...
    public function index_incity()
    {
        $booking = DB::table('bookings')->select('bookings.*', 'users.first_name', 'users.last_name', 'vehicle_types.vehicletype')
            ->where('bookings.category_id', '1')
        ...
        $data['types'] = IncCats::get();
        return view("bookings.index_in_city", $data);
    }

    public function index_delay_incity()
    {
        $data['types'] = IncCats::get();
        return view("bookings.index_in_city_delay", $data);
    }
    public function  fetch_data_incity(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $bookings->select('bookings.*', 'users.first_name', 'users.last_name', 'vehicle_types.vehicletype')
                ->where('bookings.category_id', '1')
                ->where('bookings.request_type', 'moment')
                ->join('users', 'users.id', 'bookings.user_id')
            ...
            return DataTables::eloquent($bookings)
                ...
                ->addColumn('check', function ($row) {
                    $tag = '<input type="checkbox" name="ids[]" value="' . $row->id . '" class="checkbox" id="chk' . $row->id . '" onclick=\'checkcheckbox();\'>';
               ...
               ->addColumn('car_option', function ($row) {
                     if($row->option_id== null){
                       
                        
                          return 'Not Options Selected';
                     }
                    else
                     {
                         $options_id=explode(',',$row->option_id);
                ...
                ->editColumn('km', function ($row) {
                    return intval($row->km);
                })
                ...
                ->rawColumns(['check', 'action', 'check', 'pickup_addr', 'dest_addr'])
                ->make(true);
            // return datatables(User::all())->toJson();
        }
    }

    public function fetch_delay_incity(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $bookings->select('bookings.*', 'users.first_name', 'users.last_name', 'vehicle_types.vehicletype')
                ->where('bookings.category_id', '1')
                ->where('bookings.request_type', 'delayed')
                ->join('users', 'users.id', 'bookings.user_id')
                ->join('vehicle_types', 'bookings.type_id', 'vehicle_types.id')
    ...
    public function fetch_data(Request $request)
    {
        if ($request->ajax()) {
            ...
            //     $bookings = Bookings::whereIn('vehicle_id', $vehicle_ids)->latest();
            // }
            $bookings=Bookings::select('bookings.*', 'users.first_name', 'users.last_name', 'vehicle_types.vehicletype', 'vehicles.car_model', 'subcategories.title')
                ->where('bookings.category_id', '2')
                ->join('users', 'users.id', 'bookings.user_id')
                ->join('vehicles', 'vehicles.id', 'bookings.vehicle_id')
                ->join('vehicle_types', 'vehicles.type_id', 'vehicle_types.id')
            ...
            return DataTables::eloquent($bookings)
                ...
                ->addColumn('tour', function ($row) {
                    ...
                    $tour = Tour::where('trip_id', '=', $row->id)->get();
                    if ($tour->count() > 0) {
    ...
    {
        $data['trip'] = Bookings::find($id);

        return view('bookings.add_expense', $data);
    }
    public function store_expense(Request $request)
    {
        ...
        $trip_expense->expense_type = "constant";
  ...
  {
    $data['booking']=Bookings::where('id',$id)->first();
    $data['expenses']=Expense::where('trip_id',$id)->where('expense_type','constant')->first();
  
     return view('bookings.edit_expense', $data);
      ...
      public function update_expense(Request $request)
    {
        ...
        $trip_expense = Expense::where('trip_id',$request->trip_id)->where('expense_type','constant')->first();
...
//show trip details.
    public function edit($id)
    {
        ...
        $user = User::where('id', $booking->driver_id)->first();

        $user_details=User::where('id',$booking->user_id)->first();
       
        $tour = Tour::where('trip_id', $id)->first();
        $expense = Expense::where('trip_id', $booking->id)->where('expense_type','constant')->first();
       ...
       $external=Value::where('name','external_fare')->first();
        ...
        return view("bookings.edit", compact("booking", 'tour', 'user', 'expense','user_details','internal','external'));
    }
    ...
    public function destroy(Request $request)
    {
      ...
      ReviewModel::where('booking_id',$request->get('id'))->delete();
    ...
    public function update(Request $request)
    {
        ...
        $subcategory=SubCategory::where('id',$booking->subcategory_id)->first();
     
        $booking->driver_id = $request->get('driver_id');
        $booking->save();
       $driver_token=User::where('id',$booking->driver_id)->select('device_token')->first();
      
        $str1 = " لتم اختيارك من قبل موزع الحركة  للقيام برحلة الى محافظة " ;
        $str2 = "بتاريخ";
        $body=$str1.' '.$subcategory->title.' '.$str2.' '.$booking->date;
