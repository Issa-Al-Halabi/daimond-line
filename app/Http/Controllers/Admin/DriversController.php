...
class DriversController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $users = User::where('user_type', '=', 'external_driver');
            ...
            return DataTables::eloquent($users)
                ...
                ->rawColumns(['profile_image', 'action', 'check', 'first_name'])
                ->make(true);
    ...
    public function show($id)
    {
       ...
       $index['date'] = Carbon::createFromFormat('d/m/Y', $index['created_at']);
    ...
    public function destroy(Request $request)
    {
       ...
       foreach($driver->vehicles as $vehicle){
        ...
        $driver->delete();
        $wallet=WalletModel::where('driver_id',$request->id)->delete();
      $location=DriverLocation::where('driver_id',$request->id)->delete();
    ...
    public function edit(User $driver)
    {
     ...
     $data['phone_code'] = $this->phone_code;
        ...
        return view("drivers.edit", compact("driver", 'vehicles', 'data'));
    ...
    public function store(Request $request)
    {
     ...
     'password'=>'required|min:8|regex:/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d).+$/',
        ...
        $user->in_service = "off";
        $user->phone = $request->get("phone");
        $user->join_date = $request->get("start_date");
        $user->address = $request->get("address");
