...
class HomeController extends Controller
{
    ...
    {
      $index['insidecity']=Bookings::orderBy('id', 'desc')->where('category_id', '1')->get()->count();
      $index['outcity']=Bookings::orderBy('id', 'desc')->where('category_id', '2')->get()->count();
        ...
        if (Auth::user()->user_type == "D") {
            ...
            $index['years'] = $y;
            $index['drivers'] = User::whereUser_type("driver")->get()->count();
            $index['reviews'] = ReviewModel::all()->count();
            $index['customers'] = User::where("user_type","external_driver")->get()->count();
            $index['users'] = User::whereUser_type("user")->orwhere('user_type',"foreign user")->get()->count();
            $vehicle_ids = array(0);
