...
@section('breadcrumb')
    <li class="breadcrumb-item active">
        @lang('fleet.vehicle_inspection')</li>
@endsection
@section('extra_css')
    <style type="text/css">
        .checkbox,
            ...
            <div class="card card-info">
                <div class="card-header">
                    <h3 class="card-title">
                        ...
                        &nbsp;
                        @can('VehicleInspection add')
                            <a href="{{ url('admin/vehicle-reviews-create') }}" class="btn btn-success"
                                title="@lang('fleet.add_vehicle_inspection')"><i class="fa fa-plus"></i></a>
                        @endcan
                    </h3>
                </div>
                    ...
                    <table class="table" id="ajax_data_table" style="padding-bottom: 25px">
                        ...
                        <tbody>
                            @foreach ($reviews as $r)
                                <tr>
                                    <td>
                                        <input type="checkbox" name="ids[]" value="{{ $r->id }}" class="checkbox"
                                            id="chk{{ $r->id }}" onclick='checkcheckbox();'>
                                    </td>
                                    <td>{{ $r->vehicle->maker->make }} - {{ $r->vehicle->vehiclemodel->model }} -
                                        {{ $r->vehicle->types['displayname'] }}</td>
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            ...
                                            <div class="dropdown-menu custom" role="menu">
                                                @can('VehicleInspection edit')
                                                    <a class="dropdown-item"
                                                        href="{{ url('admin/vehicle-review/' . $r->id . '/edit') }}"> <span
                                                            aria-hidden="true" class="fa fa-edit"
                                                            style="color: #f0ad4e;"></span> @lang('fleet.edit')</a>
                                                @endcan
                                                {!! Form::hidden('id', $r->id) !!}
                                                @can('VehicleInspection delete')
                                                    <a class="dropdown-item" data-id="{{ $r->id }}" data-toggle="modal"
                                                        data-target="#myModal"><span aria-hidden="true" class="fa fa-trash"
                                                            style="color: #dd4b39"></span> @lang('fleet.delete')</a>
                                                @endcan
                                                <a class="dropdown-item"
                                                    href="{{ url('admin/view-vehicle-review/' . $r->id) }}">
                                                    <span class="fa fa-eye" aria-hidden="true"
                                                        style="color: #398439"></span> @lang('fleet.view')
                                                </a>
                                            </div>
                                        </div>
                                        {!! Form::open([
                                            'url' => 'admin/delete-vehicle-review/' . $r->id,
                                            'method' => 'DELETE',
